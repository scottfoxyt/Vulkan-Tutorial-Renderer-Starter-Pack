...
void Renderer::recreateSwapchain() {
    ...
    while (width == 0 || height == 0) {
        SDL_GetWindowSize(window.getWindow(), &width, &height);
        SDL_WaitEvent(nullptr); // SDL_WaitEvent until we have a non-minimized sdlWindow size
    }
...
void Renderer::createInstance() {
    ...
    appInfo.pApplicationName = "Vulkan Tutorial Renderer Starter Pack";
...
void Renderer::createSurface() {
    if (!SDL_Vulkan_CreateSurface(window.getWindow(), instance, nullptr, &surface)) {
        throw std::runtime_error("failed to create sdlWindow surface!");
    }
}
...
void Renderer::createGraphicsPipeline() {
    auto vertShaderCode = readFile("shaders/default_shader.vert.spv");
    auto fragShaderCode = readFile("shaders/default_shader.frag.spv");
...
std::vector<const char*> Renderer::getRequiredExtensions() {
    ...
    if (!SDL_Vulkan_GetInstanceExtensions(&sdlExtensionCount)) {
        throw std::runtime_error("Could not get the required instance extensions from SDL.");
    }
